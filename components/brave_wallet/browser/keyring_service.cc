...
mojom::AllAccountsInfoPtr KeyringService::GetAllAccountsSync() {
  ...
  auto eth_dapp_selected = GetSelectedEthereumDappAccount();
  auto sol_dapp_selected = GetSelectedSolanaDappAccount();

  return mojom::AllAccountsInfo::New(
      std::move(account_infos), GetSelectedWalletAccount()->account_id.Clone(),
      eth_dapp_selected ? eth_dapp_selected->account_id.Clone() : nullptr,
      sol_dapp_selected ? sol_dapp_selected->account_id.Clone() : nullptr);
}
...
bool KeyringService::SetSelectedAccountSync(mojom::AccountIdPtr account_id) {
  ...
  for (const mojom::AccountInfoPtr& info : infos) {
    if (info->account_id == account_id) {
      SetSelectedWalletAccountInternal(*info);
      return true;
    }
  }
...
void KeyringService::MaybeFixAccountSelection() {
  ...
  if (!unique_keys.contains(GetSelectedDappAccountFromPrefs(
    ...
    // Legacy behavior when we always have ETH dApp account selected.
    NOTREACHED();
  }
