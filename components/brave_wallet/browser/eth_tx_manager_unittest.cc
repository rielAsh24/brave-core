...
void AddUnapprovedTransactionSuccessCallback(base::RunLoop* run_loop,
...
void AddUnapprovedTransactionFailureCallback(base::RunLoop* run_loop,
                                             bool success,
                                             const std::string& id,
                                             const std::string& error_message) {
 ...
 public:
  ...
  void SetUp() override {
    ...
    tx_service_ = std::make_unique<TxService>(
        json_rpc_service_.get(), nullptr, keyring_service_.get(),
  ...
  void DoSpeedupOrCancelTransactionSuccess(
    ...
    eth_tx_manager()->SpeedupOrCancelTransaction(
        ...
        base::BindOnce(&AddUnapprovedTransactionSuccessCallback, &run_loop,
                       tx_meta_id));
    run_loop.Run();
  }
  ...
  void DoSpeedupOrCancelTransactionFailure(const std::string& chain_id,
    ...
    eth_tx_manager()->SpeedupOrCancelTransaction(
        ...
        base::BindOnce(&AddUnapprovedTransactionFailureCallback, &run_loop));
    run_loop.Run();
  }
  ...
  std::unique_ptr<EthTxMeta> GetTxForTesting(const std::string& chain_id,
    ...
    std::unique_ptr<EthTxMeta> meta_out = nullptr;
    base::RunLoop run_loop(run_loop_nested ? kNestableTasksAllowed : kDefault);
    eth_tx_manager()->GetTxForTesting(
        chain_id, tx_meta_id,
        base::BindLambdaForTesting([&](std::unique_ptr<EthTxMeta> meta) {
  ...
  void AddOrUpdateTx(const TxMeta& meta) {
    base::RunLoop run_loop;
    eth_tx_manager()->tx_state_manager_->AddOrUpdateTx(
...
TEST_F(EthTxManagerUnitTest, AddUnapprovedTransactionWithGasPriceAndGasLimit) {
  ...
  run_loop.Run();
  auto tx_meta = GetTxForTesting(mojom::kLocalhostChainId, tx_meta_id);
  EXPECT_TRUE(tx_meta);
...
TEST_F(EthTxManagerUnitTest,
  ...
  AddUnapprovedTransaction(
      ...
      base::BindOnce(&AddUnapprovedTransactionSuccessCallback, run_loop.get(),
                     &tx_meta_id),
      "mockGroupId");
  run_loop->Run();
  auto tx_meta = GetTxForTesting(mojom::kLocalhostChainId, tx_meta_id);
  ...
  AddUnapprovedTransaction(
      ...
      base::BindOnce(&AddUnapprovedTransactionSuccessCallback, run_loop.get(),
                     &tx_meta_id));
  run_loop->Run();
  tx_meta = GetTxForTesting(mojom::kLocalhostChainId, tx_meta_id);
  EXPECT_TRUE(tx_meta);
...
TEST_F(EthTxManagerUnitTest, AddUnapprovedTransactionWithoutGasPrice) {
  ...
  AddUnapprovedTransaction(
      mojom::kLocalhostChainId, std::move(tx_data), from(),
      base::BindOnce(&AddUnapprovedTransactionFailureCallback, run_loop.get()));
  run_loop->Run();
}
...
TEST_F(EthTxManagerUnitTest, ProcessHardwareSignature) {
  ...
  eth_tx_manager()->ProcessHardwareSignature(
      ...
      base::BindLambdaForTesting([&](bool success,
        ...
        auto tx_meta =
            GetTxForTesting(mojom::kLocalhostChainId, tx_meta_id, true);
        EXPECT_TRUE(tx_meta);
...
TEST_F(EthTxManagerUnitTest, TestSubmittedToConfirmed) {
  ...
  auto tx_meta1 = GetTxForTesting(mojom::kLocalhostChainId, "001");
  EXPECT_EQ(mojom::TransactionStatus::Submitted, tx_meta1->status());
  auto tx_meta2 = GetTxForTesting(mojom::kMainnetChainId, "002");
  EXPECT_EQ(mojom::TransactionStatus::Submitted, tx_meta2->status());

  task_environment_.FastForwardBy(base::Seconds(2));
  tx_meta1 = GetTxForTesting(mojom::kLocalhostChainId, "001");
  EXPECT_EQ(mojom::TransactionStatus::Confirmed, tx_meta1->status());
  tx_meta2 = GetTxForTesting(mojom::kMainnetChainId, "002");
...
TEST_F(EthTxManagerUnitTest, SpeedupTransaction) {
  ...
  auto expected_tx_meta =
      GetTxForTesting(mojom::kLocalhostChainId, orig_meta_id);
  ...
  expected_tx_meta = GetTxForTesting(mojom::kLocalhostChainId, orig_meta_id);
  ASSERT_TRUE(expected_tx_meta);
  expected_tx_meta->tx()->set_gas_price(103027933985ULL);  // 0x17fcf18321
  tx_meta = GetTxForTesting(mojom::kLocalhostChainId, tx_meta_id);
  ASSERT_TRUE(tx_meta);
...
TEST_F(EthTxManagerUnitTest, CancelTransaction) {
  ...
  auto orig_tx_meta = GetTxForTesting(mojom::kLocalhostChainId, orig_meta_id);
  ...
  orig_tx_meta = GetTxForTesting(mojom::kLocalhostChainId, orig_meta_id);
  ASSERT_TRUE(orig_tx_meta);
  tx_meta = GetTxForTesting(mojom::kLocalhostChainId, tx_meta_id);
  ASSERT_TRUE(tx_meta);
  EXPECT_EQ(tx_meta->tx()->nonce(), orig_tx_meta->tx()->nonce());
